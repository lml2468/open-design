...
export function SettingsDialog({
  ...
  useEffect(() => {
    ...
    setCfg((current) => {
      if (!parentPrivacyChanged) return current;
      return {
        ...current,
  ...
  const installedCount = useMemo(
    () => agents.filter((a) => a.available && isVisibleSettingsAgent(a)).length,
    [agents],
  );
    ...
    try {
      ...
      setAgentRescanNotice({
        ...
        count: nextAgents.filter(
          (agent) => agent.available && isVisibleSettingsAgent(agent),
        ).length,
      });
    } catch {
  ...
  const renderAgentModelConfig = (selected: AgentInfo) => {
    ...
    // model-selection flag or rejects free-text model ids. Undefined means
    // allow, matching the existing runtime contract.
    ...
    const modelValue = customModelDraft ?? configuredModel ?? defaultAgentModelId(selected) ?? '';
        ...
        <div className="modal-body">
          ...
          {cfg.mode === 'daemon' ? (
            <section className="settings-section">
              ...
              {initialAgentScanRunning ? (
                ...
                <>
                  ...
                  {/*
                    ...
                    noise.
                  */}
                  {!visibleAgents.find((a) => a.id === cfg.agentId && a.available) ? (
              ...
              {(() => {
                const selected = visibleAgents.find(
