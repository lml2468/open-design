...
// legacy sync runs for any active live workspace, including a personal
...
export interface CollabSessionDecision {
  /** Whether to start the legacy sync-status poll. */
  enabled: boolean;
...
/**
 * Decide whether legacy collaboration sync should run for the current
 * workspace context. Gating (in order):
 * - no context → off
 * - removed member → off
...
export function resolveCollabSession(ctx: WorkspaceCollabContext | null): CollabSessionDecision {
  if (!ctx) return { enabled: false, reason: 'no-workspace-context' };
  if (ctx.memberStatus !== 'active') {
    return { enabled: false, reason: 'member-removed' };
  }
  if (!LIVE_LIFECYCLE.has(ctx.lifecycleState)) {
    return { enabled: false, reason: `lifecycle-${ctx.lifecycleState}` };
  }
