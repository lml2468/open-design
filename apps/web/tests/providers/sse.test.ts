...
describe('streamViaDaemon', () => {
  ...
  it('preserves structured SSE error codes and action details', async () => {
    const handlers = createDaemonHandlers();
    vi.stubGlobal(
      'fetch',
      vi.fn()
        ...
        .mockResolvedValueOnce(
          sseResponse(
            [
              'event: error',
              'data: {"message":"Provider temporarily unavailable","error":{"code":"UPSTREAM_UNAVAILABLE","message":"Provider temporarily unavailable","details":{"kind":"provider","retryable":true}}}',
              '',
              '',
            ].join('\n'),
          ),
