...
describe('composeSystemPrompt', () => {
  ...
  it('keeps media routing provider-neutral and forbids reading generated bytes back into context', () => {
    const prompt = composeSystemPrompt({
      skillMode: 'image',
      metadata: {
        kind: 'image',
  ...
  describe('artifact handoff no-emit clauses (#1143)', () => {
    ...
    it('pins filesystem artifact handoff for filesystem runs', () => {
      const prompt = composeSystemPrompt({});
    ...
    it('uses only run-scoped media defaults without agent-specific routing', () => {
      const prompt = composeSystemPrompt({
        agentId: 'codex',
    ...
    it('keeps image completion copy concrete while retaining internal diagnostics', () => {
      ...
      expect(prototypePrompt).toContain('IMAGE_MODEL="gpt-image-2"');
    });
    ...
    it('prioritizes question forms over native tool calls when clarifying', () => {
      const prompt = composeSystemPrompt({ agentId: 'codex' });
