...
describe('resolveModelForAgent', () => {
  ...
  it('prefers an enabled default remembered model over a disabled first catalog entry', () => {
    const def = defWithId('catalog-disabled-default-test', []);
    const models = [
  ...
  it('uses the first enabled remembered model when no enabled model is marked default', () => {
    const def = defWithId('catalog-disabled-first-test', []);
    rememberLiveModels(def.id, [
      { id: 'locked-upgrade-model', label: 'Locked', enabled: false },
      { id: 'enabled-model', label: 'Enabled', enabled: true },
    ]);
  ...
  it('isolates remembered live models by environment profile scope', () => {
    const def = defWithId('catalog-agent', []);
    rememberLiveModels(def.id, [
  ...
  it('prefers remembered live models only when the fresh catalog is empty', () => {
    const remembered = [
      { id: 'deepseek-v3.2', label: 'deepseek-v3.2' },
      { id: 'glm-5.1', label: 'glm-5.1' },
    ];
  ...
  it('honors defaultModelEnvVar over the hardcoded fallback when no model is set', () => {
    const def: RuntimeAgentDef = {
      ...
      defaultModelEnvVar: 'TEST_DEFAULT_MODEL',
    };
    expect(
      resolveModelForAgent(def, null, { TEST_DEFAULT_MODEL: 'gpt-5.5' }),
    ).toBe('gpt-5.5');
    expect(
      resolveModelForAgent(def, 'default', { TEST_DEFAULT_MODEL: 'gpt-5.5' }),
    ).toBe('default');
  });
  ...
  it('falls back to the static list when defaultModelEnvVar is set but the env var is empty / missing', () => {
    ...
    expect(resolveModelForAgent(def, null, {})).toBe('gpt-5.4-mini');
    expect(
      resolveModelForAgent(def, null, { TEST_DEFAULT_MODEL: '   ' }),
    ).toBe('gpt-5.4-mini');
  });
  ...
  it('does NOT use the env override when the user already picked a real model', () => {
    ...
    expect(
      resolveModelForAgent(def, 'gpt-5.4-fast', { TEST_DEFAULT_MODEL: 'gpt-5.5' }),
    ).toBe('gpt-5.4-fast');
  });
});
