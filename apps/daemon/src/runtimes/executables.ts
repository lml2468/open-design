...
export function inspectAgentExecutableResolution(
  ...
  // candidates are skippable: an explicit `*_BIN` override and the Codex app
  // bundle are deliberate selections, so a
  ...
  return {
    ...
    pathResolvedPath,
    selectedPath:
      configuredOverridePath || pathResolvedPath || appBundlePath,
  };
}
