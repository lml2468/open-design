...
function stableRunIngestionEventId(event: unknown): string | null {
  ...
  return `od-${createHash('sha256')
    .update(
      // Keep the pre-removal final-delivery namespace so existing Langfuse
      // events retain their stable ingestion identity across upgrades.
...
function stabilizeRunIngestionBatch(batch: unknown[]): unknown[] {
  return batch.map((event) => {
    const id = stableRunIngestionEventId(event);
...
export function buildTracePayload(ctx: ReportContext): unknown[] {
  ...
  return stabilizeRunIngestionBatch(batch);
}
