...
// behavioural funnel by BYOK / CLI. `byok` is the user's own key (web-only
// visibility — see the daemon note in `AnalyticsConfigureGlobals`),
// `local_cli` is a detected local coding CLI.
...
export interface AnalyticsConfigureGlobals {
  ...
  // only layer that knows whether the run used BYOK.
  runtime_type: TrackingRuntimeType;
  // Per-path "reached a runnable/usable state" flags. Unlike `configure_type`
  // — a single priority cascade (both > local_cli > byok) where a value masks
  // the lower-priority path — these two are INDEPENDENT booleans, so
  ...
  // count the success result events instead — `settings_cli_test_result` and
